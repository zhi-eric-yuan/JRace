...
public class BOBYQAOptimizer
	 ...
	 */
	public ArrayRealVector getCurrentBest() {
		return currentBest;
	}
    ...
    private double bobyqb(double[] lowerBound,
        ...
        for(;;) {
        	if (function instanceof AlgorithmEvaluator && 
                ...
                // Something wrong, probably by checking max consecutive archive read from algorithm evaluator
        		log.error("Maximum consecutive archive read reached, stop current iteration");
        		if (f != ZERO) {
        			return f;
        		} else {
	        ...
	        case 360: {
	            ...
	            if (f < fopt) {
	                ...
	                // Edited by Eric. To output the runtime development data
	                double[] newBestOverTime = {getEvaluations(), f};
	                bestOverTime.add(newBestOverTime.clone());
	            	historicalBest.add(currentBest.toArray());
	                // This is only for additional evaluation for historical best
	            	//incrementEvalCount();
	
	                /*OutputHandler.out.append(evaluations.getCount());
	                OutputHandler.out.append(" ");
	                OutputHandler.out.append(f);
	                OutputHandler.out.append("\n");*/
	            } 
	            
	            updateBest(f, currentBest.toArray());
    ...
    private void prelim(double[] lowerBound,
        ...
        do {
        	...
        	final int nfm = numEvalPoints;
            //System.out.println("nfm " + nfm + " " + getEvaluations());
            final int nfx = nfm - n;
            ...
            final int numEval = numEvalPoints; // nfm + 1
            ...
            if (numEval == 1) {
                ...
                trustRegionCenterInterpolationPointIndex = 0;
                // Edited by Eric. To output the runtime development data
                //double[] newBestOverTime = {numEvalPoints, f};
                //bestOverTime.add(newBestOverTime.clone());
                //bestPoint = currentBest.getDataRef().clone();
                //bestValue = f;
